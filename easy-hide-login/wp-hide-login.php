...
class Easy_Hide_Login
{
    static $version;
    static $options;
    static $login_access_granted = false;
    ...
    static function init()
    {
        ...
        if (
            !empty($options['slug']) &&
            is_string($query) &&
            hash_equals((string) $options['slug'], urldecode($query))
        ) {
        ...
        if (is_admin()) {
            ...
            if (!empty($options['slug'])) {
                ...
                add_filter('lostpassword_url', array(__CLASS__, 'lostpassword'), 10, 1);
    ...
    static function site_url($url, $path, $scheme, $blog_id)
    {
        ...
        // Never disclose the hidden slug to an anonymous request
        // that has not already passed the login slug gate.
        if (!self::$login_access_granted) {
            return $url;
        }

        return add_query_arg($options['slug'], 1, $url);
    }

    static function request_has_login_slug()
    {
        $options = self::get_options();
        ...
        foreach ($query_params as $param) {
            ...
            if (
                hash_equals((string) $options['slug'], $key) &&
                ($value === '' || $value === '1')
            ) {
                return true;
            }
    ...
    static function login_head()
    {
        ...
        $has_valid_slug = self::request_has_login_slug();

        if (
            !$has_valid_slug &&
            isset($_POST['redirect_slug']) &&
            hash_equals(
                (string) $options['slug'],
                sanitize_text_field(wp_unslash($_POST['redirect_slug']))
            )
        ) {
            $has_valid_slug = true;
        }
        ...
        self::$login_access_granted = true;

        if (
            isset($_GET['action']) &&
            sanitize_text_field(wp_unslash($_GET['action'])) === 'logout'
        ) {
            check_admin_referer('log-out');
    ...
    static function lostpassword($lostpassword_url)
    {
        if (!self::$login_access_granted) {
                ...
                if (isset($_POST['slug'])) {
                    ...
                    if (in_array($slug, $reserved_slugs, true)) {
                        echo '<div id="message" class="error fade">
                            <p><strong>' .
                            esc_html__('Please choose a different login slug. This value is reserved by WordPress.', 'easy-hide-login') .
                            '</strong></p>
                        </div>';
                    } else if ($slug !== '' && !preg_match('/^[a-zA-Z0-9_-]+$/', $slug)) {
                        echo '<div id="message" class="error fade">
                            <p><strong>' .
                            esc_html__('The login slug contains invalid characters.', 'easy-hide-login') .
                            '</strong></p>
                        </div>';
                    } else {
                        $options['options']['slug'] = $slug;
                        $change = true;

                        echo '<div id="message" class="updated fade">
                            <p><strong>' .
                            esc_html__('Options saved.', 'easy-hide-login') .
                            '</strong></p>
                        </div>';
                    }
                }
    ...
    static function options_page()
    {
        ...
        echo '<td><input type="text" id="login_slug" value="' . esc_attr($options['slug']) . '" name="slug"><p>Your login slug can contain letters, numbers, hyphens (-), and underscores (_) only.</p></td>';
        echo '</tr>';
        echo '<tr>';
